...
const AddLandmarkModal: React.FC<Props> = ({ isOpen, onClose }) => {
  const [landmarkType, setLandmarkType] = useState('Hospital');
  ...
  const selectLandmarkType = (type: string) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center ">
      <div className="bg-white rounded-xl border w-full max-w-2xl relative shadow-lg max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-6">
          {/* Landmark Type and Distance Row */}
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <div className="relative">
                <button
                  onClick={() => setShowLandmarkDropdown(!showLandmarkDropdown)}
                ...
                >
                  <span className="text-gray-900">{landmarkType}</span>
                ...
                {showLandmarkDropdown && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg max-h-40 overflow-y-auto">
                    {landmarkTypes.map((type) => (
                      <button
                        key={type}
                        onClick={() => selectLandmarkType(type)}
...
const AddLandmarkDemo: React.FC = () => {
...
export default AddLandmarkDemo;
export { AddLandmarkModal };